    ...
    if (options.customFormat) {
        Object.keys(options.customFormat).forEach(function forEachCallback(item) {
            if (include(item) && json[item]) {
                ...
                switch(item) {
                    case "publisher":
                        moduleInfo[item] = json.author !== undefined ? (json.author.publisher || options.customFormat[item]) : options.customFormat[item]
                        break;
                    case "email":
                        moduleInfo[item] = json.author !== undefined ? (json.author.email || options.customFormat[item]) : options.customFormat[item]
                        break;
                    case "url":
                        moduleInfo[item] = json.author !== undefined ? (json.author.url || options.customFormat[item]) : options.customFormat[item]
                        break;
                    default:
