    ...
    if (options.customFormat) {
        Object.keys(options.customFormat).forEach(function forEachCallback(item) {
            if (include(item) && json[item]) {
                ...
                switch(item) {
                    case "publisher":
                        moduleInfo[item] = if (json.author) ? (json.author.publisher || options.customFormat[item]) : options.customFormat[item]
                        break;
                    case "email":
                        moduleInfo[item] = if (json.author) ? (json.author.email || options.customFormat[item]) : options.customFormat[item]
                        break;
                    case "url":
                        moduleInfo[item] = if (json.author) ? (json.author.url || options.customFormat[item]) : options.customFormat[item]
                        break;
                    default:
